...
//@access   PUBLIC
router.get("/", (req, res) => res.json({ test: "auth test successfull" }));
...
//@desc     routh for registration of users
//@access   PUBLIC
  ...
  Person.findOne({ email })
    .then((person) => {
      if (!person) {
        return res
          .status(404)
          .json({ emailerror: "user nor found with this email" });
      }
      bcrypt
